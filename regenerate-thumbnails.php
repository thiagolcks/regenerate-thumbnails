...
<?php
	...
	public function ajax_process_image() {
		...
		if ( ! current_user_can( $this->capability ) )
			return array( 'success'=>false, 'message'=>__( "Your user account doesn't have permission to resize images", 'regenerate-thumbnails' ) );

		$result = $this->process_image( $id );
		
		if ( isset( $result['helper_error'] ) )
			$this->die_json_error_msg( $id, $result['error'] );
		else
	...
	public function process_image( $id ) {
		
		$image = get_post( $id );

		if ( ! $image || 'attachment' != $image->post_type || 'image/' != substr( $image->post_mime_type, 0, 6 ) )
			return( array( 'error' => sprintf( __( 'Failed resize: %s is an invalid image ID.', 'regenerate-thumbnails' ), esc_html( $id ) ) ) );

		$fullsizepath = get_attached_file( $image->ID );

		if ( false === $fullsizepath || ! file_exists( $fullsizepath ) )
			return array( 'error'=> sprintf( __( 'The originally uploaded image file cannot be found at %s', 'regenerate-thumbnails' ), '<code>' . esc_html( $fullsizepath ) . '</code>' ), 'helper_error'=>true );
		...
		if ( is_wp_error( $metadata ) )
			return array( 'error'=>$metadata->get_error_message(), 'helper_error'=>true );
		if ( empty( $metadata ) )
			return array( 'error'=>__( 'Unknown failure reason.', 'regenerate-thumbnails' ), 'helper_error'=>true );

		// If this fails, then it just means that nothing was changed (old value == new value)
		wp_update_attachment_metadata( $image->ID, $metadata );

		return array( 'success' => sprintf( __( '&quot;%1$s&quot; (ID %2$s) was successfully resized in %3$s seconds.', 'regenerate-thumbnails' ), esc_html( get_the_title( $image->ID ) ), $image->ID, timer_stop() ) );
	}
